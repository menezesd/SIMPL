...
object IdiomaticCodegen:
  ...
  def emitExpr(e: id.Expr, ctx: Ctx): String = e match
    ...
    case id.Var(name, _) =>
      if ctx.frameOpt.nonEmpty then
        ...
        val vb = vbOpt.getOrElse(throw new CompilerException("Undeclared variable: " + name, -1))
    ...
    case id.Binary(op, left, right, _, _) =>
      ...
      val ch = op match
        ...
        case BinaryOp.Lt  => '<'
        case BinaryOp.Gt  => '>'
        case BinaryOp.Le  => throw new CompilerException("'<= not supported by runtime'", -1)
        case BinaryOp.Ge  => throw new CompilerException(">= not supported by runtime", -1)
        case BinaryOp.Ne  => throw new CompilerException("!= not supported by runtime", -1)
        case BinaryOp.Concat => throw new CompilerException("concat is internal-only for strings", -1)
    ...
    case id.This(_) =>
      ...
      if ctx.frameOpt.nonEmpty then
        val vb = ctx.frameOpt.get.lookupVar("this").getOrElse(throw new CompilerException("'this' not found in current frame", -1))
        s"PUSHOFF ${vb.getAddress}\n"
      else
  ...
  def emitStmt(s: id.Stmt, ctx: Ctx): String = s match
    ...
    case id.Assign(name, value, _) =>
      if ctx.frameOpt.isEmpty then throw new CompilerException("Missing frame for assignment codegen", -1)
      val vb = ctx.frameOpt.get.lookupVar(name).getOrElse(throw new CompilerException("Undeclared variable: " + name, -1))
    ...
    case id.Return(valueOpt, _) =>
      ...
      val ret = ctx.returnLabelOpt.getOrElse(throw new CompilerException("Return label not found in scope", -1))
      sb.append("JUMP ").append(ret.getName).append("\n").toString
