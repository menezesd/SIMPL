  ...
  def emitExprD(e: id.Expr, ctx: Ctx): Either[Diag, Code] =
    e match
      ...
      case id.Unary(op, expr, rt, pos) =>
        op match
          case UnaryOp.Neg =>
            if rt.exists(_ == Type.STRING) || tOf(expr, ctx) == Type.STRING then Right(emitExprCore(e, ctx))
            else Right(emitExprCore(e, ctx)) // int neg handled in core; nothing to diagnose here
          case UnaryOp.Not => Right(emitExprCore(e, ctx))
      case id.Binary(op, left, right, _, pos) =>
        val lt = tOf(left, ctx); val rt = tOf(right, ctx)
        // String-sensitive diagnostics first
        op match
          case BinaryOp.Add if lt == Type.STRING || rt == Type.STRING =>
            if lt == Type.STRING && rt == Type.STRING then Right(emitExprCore(e, ctx))
            else Left(TypeDiag("'+' only defined for String+String or numeric addition", pos))
          case BinaryOp.Mul if lt == Type.STRING || rt == Type.STRING =>
            val ok = (lt == Type.STRING && rt == Type.INT) || (lt == Type.INT && rt == Type.STRING)
            if ok then Right(emitExprCore(e, ctx)) else Left(TypeDiag("Repeat requires (String*Int) or (Int*String)", pos))
          case BinaryOp.Lt | BinaryOp.Gt | BinaryOp.Eq if lt == Type.STRING || rt == Type.STRING =>
            if lt == Type.STRING && rt == Type.STRING then Right(emitExprCore(e, ctx))
            else Left(TypeDiag("String comparison requires both operands String", pos))
          case BinaryOp.Le | BinaryOp.Ge | BinaryOp.Ne if lt == Type.STRING || rt == Type.STRING =>
            Left(TypeDiag("Unsupported string operator", pos))
          case BinaryOp.Concat => Left(TypeDiag("'concat' is internal-only for strings", pos))
          case _ => Right(emitExprCore(e, ctx))
      case id.Call(_, _, _) | id.Ternary(_, _, _, _, _) | id.IntLit(_, _) | id.BoolLit(_, _) | id.StrLit(_, _) | id.NullLit(_) | id.This(_) | id.NewObject(_, _, _) | id.InstanceCall(_, _, _, _) =>
        Right(emitExprCore(e, ctx))
  ...
  def emitStmtD(s: id.Stmt, ctx: Ctx): Either[Diag, Code] =
    s match
      case id.Assign(name, value, pos) =>
        ctx.frameOpt match
          case Some(frame) =>
            frame.lookupVar(name).toRight(ResolveDiag(s"Undeclared variable: $name", pos)).map { vb =>
              val sb = new SamBuilder()
              sb.append(emitExprCore(value, ctx)).storeOffS(StackOffset(vb.getAddress))
              Code.from(sb)
            }
      ...
      case id.FieldAssign(target, field, offset, value, pos) =>
        ...
        else Right(emitStmtCore(s, ctx))
      case id.Break(pos) =>
        if ctx.loopEndLabels.headOption.isEmpty then Left(SyntaxDiag("'break' used outside of loop", pos))
        else Right(emitStmtCore(s, ctx))
      case id.Return(_, pos) =>
        if ctx.returnLabelOpt.isEmpty then Left(ResolveDiag("Return label not found in scope", pos))
        else Right(emitStmtCore(s, ctx))
      case _ => Right(emitStmtCore(s, ctx))
